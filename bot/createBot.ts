...
import { MyContext } from "../models/MyContext";
  ...
  import { checkForUserAgentTokenGatedInfoV1 } from "../functions/checkForUserAgentTokenGatedInfo";
  import { saveMessageAndInformAI } from "../functions/saveMessageAndInformAI";
  import { getAgent } from "../utils/groups/getAgent";
  import { getGroupAgentToolInfoWithoutPrivacy, getGroupAgentToolInfoWithoutPrivacyBotToken } from "../utils/groups/getGroupAgentToolInfoWithoutPrivacy";
  import { getProjectToolsInfo, updateProjectGroupId } from "../utils/groups/getProjectToolsInfo";
...
export function createBotInstance(token: string, projectId?: string) {
  // Create bot and attach middlewares + handlers. This mirrors your single-bot setup.
  ...
  // const project_id = '6E8vh6GgnGzevfAxDc5aEwNyQuhs9tJPgWNV7xjS8ovg';
  ...
  //   console.log(ctx.chatMember.new_chat_member, "==new Member");
  //   console.log(JSON.stringify(ctx, null, 2));
  // })
  bot.on("message:text", async (ctx) => {
    console.log("Received messageddd text:", ctx.message.text, JSON.stringify(ctx, null, 2));
    const text = ctx.message.text.toLowerCase();
    const username = ctx.from.id ? ctx.from.id : "unknown_user";

    const apiUrl = "https://mmoshapi-uodcouqmia-uc.a.run.app/generate/";

    // const apiUrl = "http://0.0.0.0:8080/generate/";
    ...
    const groupAgentToolInfo = await getGroupAgentToolInfoWithoutPrivacyBotToken(
      ctx.api.token ?? "",
    );

    let agent = null;

    if (groupAgentToolInfo) {
      agent = await getAgent(groupAgentToolInfo.project);
    }

    if (["group", "supergroup"].includes(ctx.message.chat.type)) {
      console.log("Group chat detected");
      if (!ctx.from.username) return;
      console.log("Saving message and informing AI...");
      ...
      console.log("Has mention params?", hasMention);

      // if (!hasMention) return;
      console.log("Bot mentioned, proceeding to respond...");

      const finalText = ctx.message.text
        .replace(`@${bot_username}`, "")
        .replace(`@${bot_name}`, "");

      console.log(ctx.message.chat, "==chat.message11")
      console.log(ctx.message, "==chat.message.text11")

      

      let defaultReferred = process.env.NEXT_PUBLIC_GENESIS_PROFILE!;

      console.log(agent, "===agensdssdt")
      console.log(agent?.system_prompt, "===agent.systemPrompt")
      let systemPrompt = "";
      if (agent && agent.system_prompt && agent.system_prompt.length > 0) {
        systemPrompt = agent.system_prompt;
      }
      if (groupAgentToolInfo && groupAgentToolInfo.data.instructions && groupAgentToolInfo.data.instructions.length > 0) {
        systemPrompt += "\n [ADDITIONAL INSTRUCTIONS STARTS]\n";
        systemPrompt += "\n" + groupAgentToolInfo.data.instructions;
        systemPrompt += "\n [ADDITIONAL INSTRUCTIONS ENDS]\n";
      }
      if (!!agent) {
        defaultReferred = agent.creatorUsername;
      }

      console.log("Prompt making finaal>>>>")

      postData.system_prompt =
        systemPrompt +
        DEFAULT_SYSTEM_PROMPT_END.replace("referral_code", defaultReferred);
      postData.prompt = finalText;
      console.log("SYSTEM PROMPT:", postData.system_prompt);
      console.log("Final prompt:", JSON.stringify(postData, null, 2));
      ...
      try {
        console.log("Fetching from API:", apiUrl, JSON.stringify(postData, null, 2));
        let systemPrompt = "";
        if (agent && agent.system_prompt && agent.system_prompt.length > 0) {
          systemPrompt = agent.system_prompt;
        }
        ...
        postData.system_prompt =
          systemPrompt +
          DEFAULT_SYSTEM_PROMPT_END.replace(
            "referral_code",
            process.env.NEXT_PUBLIC_GENESIS_PROFILE!,
          );
        console.log("SYSTEM PROMPT private:", postData.system_prompt);
        console.log("Final prompt:", JSON.stringify(postData, null, 2));
  ...
  bot.on("message", (ctx) => checkForUserAgentTokenGatedInfoV1(ctx, projectId!));
